...
Brief Description: Contains mainly the public interface for accessing a
...
struct OAConfig
{
	...
	struct HeaderBlockInfo
	{
		...
		{
			// Assigns size based on desired header type
			if (type_ == hbBasic)
...
class ObjectAllocator
{
  ...
  private:
	...
	OAConfig mConfig;					// Configuration settings
	OAStats mStats;                     // Stored information about allocations and such
	unsigned numAllocations;            // Number of total allocations that have been given by this manager
	...
	GenericObject* grab_free_object();  // Gets an object from the freelist, or returns null if none
	
	bool validate_object_boundary(void* object); // Checks to see if the object is on a valid boundary, returns whether it is or not
	
	bool is_on_freelist(void * object) const; // Returns if the given object is on the free list
	
	// // Initilizes object header for the given object
	// (assumes start of header is the given pointer)
	void init_object_header(void* object); 

	// Gives an object it's allocation number before returning to client
	// (Expects object ptr to be at beginning of data portion)
	void set_allocation_num(void* object, bool isAllocated); 

	// Sets the allocation bit of the object's header 
	// (Expects object ptr to be at beginning of data portion)
	void set_allocated_flag(void* object, bool isAllocated); 
	...
	void increase_use_count(void* object);

	// Allocates external header, sets label, and gives to object
	// (Expects object ptr to be at the beginning of the data portion)
	void allocate_and_set_external_header(void* object, const char* label);

	//  Deallocates entire external header of object
	// (Expects object ptr to be at the beginning of the data portion)
	void deallocate_external_header(void* object);

	// Validates if the padding bytes around an object, returns false if any corruption is found
	// (Expects object ptr to be at the beginning of the data portion)
	bool validate_object_padding(void* object);
